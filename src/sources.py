...
class ApiSource:
    ...
    def get_tasks(self) -> list[Task]:
        return [
            Task(1, "сварить пельмени", 1),
            Task(2, "сделать салат", 3),
            Task(3, "испечь пирог", 2),
            Task(4, "убраться в комнате", 4),
            Task(5, "сходить в магазин", 5)
        ]
        ...
        try:
            with open(self.filename, 'r', encoding='utf-8') as file:
                for line_number, line in enumerate(file, 1):
                    ...
                    line = line.strip()
                    if line:
                        tasks.append(Task(line_number, line.strip(), priority))
